...
|---|---|
| `tests/WireTests.swift` | 198 checks: signed coordinate conversion, displays above/left/below the primary, no double Retina scaling, display selection, JSON fields including action ids and mouse bindings, popup subroles, Emacs AXTextField admission, titlebar AX/CG slop, Chrome CGWindow omission, Finder hide verification, process instance identity, fingerprint remap, key mapping, plan validation, focus-border tracing during restore, unfocused-border clip against the focused window |
